...
class ExportDriverTypeCylinderPopup extends React.Component {
    ...
    async addHistory(driver, license_plate, typeExportCylinder, number_array, toArray,
        cylindersWithoutSerial, cylinders, idDriver, sign, cylinderImex,
        idImex,
        typeImex,
        flow) {
        // Call api
    ...
    render() {
        const nameExport = this.props.typeExportCylinder === Constants.RENT ? this.props.t('PART_RENT') : this.props.typeExportCylinder === Constants.BUY ? this.props.t('PART_BUY_SOPET') : this.props.typeExportCylinder === Constants.RETURN_CYLINDER ? this.props.t('EXP_RETURN') : this.props.t('FIX_TITLE');
        const {
            listFixerExportCell,
            listRenterExportCell,
        } = this.state
        // console.log('listFixerExportCell', listFixerExportCell, this.props.listUsersPartner)
        return (
...
export default withNamespaces()(ExportDriverTypeCylinderPopup) ;
