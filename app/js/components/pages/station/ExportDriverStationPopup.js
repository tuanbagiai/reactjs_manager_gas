...
import SelectMulti from 'react-validation/build/select';
...
class ExportDriverStationPopup extends React.Component {
    ...
    async addHistory(driver, license_plate, cylinders, to_id, idDriver, sign) {

        // Call api
        this.setState({ isLoading: true });
        const user = await createHistoryAPI(driver, license_plate, cylinders, Constant.EXPORT_STATION, to_id, '', '', '', '', '', '', '', '', '', '', idDriver, sign);
        this.setState({ isLoading: false });
        //console.log('register',user);
        if (user) {
            if (user.status === 200) {
                if (user.data.hasOwnProperty("err_msg")) {
                    showToast(user.data.err_msg)
                    //that.setState({error: user.data.err_msg,listProducts:[]})
                    return
                }
    ...
    async submit(event) {
        ...
        await this.addHistory(nameDriver, data.license_plate, cylinders, data.factory, data.idDriver, sign);
                                        ...
                                        <div className="col-md-6">
                                            <div className="form-group">
                                                ...
                                                <Select 
                                                    ...
                                                    {this.props.listFactoryUser.map((item, index) => <option key={index} value={item.id}>{item.name}</option>)}
                                                </Select>
                                            </div>
                                        </div>
...
export default ExportDriverStationPopup;